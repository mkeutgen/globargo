# CARBON DETECTION ALGORITHM - CAT1 and CAT2 Events #
# MK #
...
# Read the manually classified dataframe
classified_df <- read.csv("manual_classification¨.csv")
...
for (j in seq_along(wmo_cat2)) {
  try({
    ...
    # Restrict the cycle_number to the one found by the Detection Algorithm on AOU
    # and SPIC to speed up the algorithm
    cycle_number <- cat2_events %>% filter(WMO== wmo) %>% select(CYCLE_NUMBER) %>% unique() %>% as_vector()
