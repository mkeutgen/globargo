  ...
  try({
    ...
    residuals_data_wf <- residuals_data %>%
      ...
      select(PRES_ADJUSTED, SCALE_RES_ROB,VALUE, VAR, CYCLE_NUMBER, LONGITUDE,
             LATITUDE, TIME) %>%
      pivot_wider(names_from = VAR, values_from = c(SCALE_RES_ROB,VALUE)) %>%
      mutate(OUT_S = ifelse(abs(SCALE_RES_ROB_AOU) > cutoff
    ...
    if (length(list_plots) > 0) {
      dir <- paste0("/data/GLOBARGO/figures/EddySubductionFiguresAOU_Only/", wmo)
      if (!dir.exists(dir)) {
...
write_csv(detected.events.df, "/data/GLOBARGO/data/detected_events_aou_only.csv")
