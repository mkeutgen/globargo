...
carbon_cat1$Category <- 1
carbon_cat2$Category <- 2
carbon_merged <- rbind(carbon_cat1,carbon_cat2)
write_csv(carbon_merged,"/data/GLOBARGO/data/detected_events_carbon.csv")


# Investigation of Eastern Pacific Events :
carbon_merged %>% filter(LONGITUDE > -150) %>%
  filter(LONGITUDE < -80 ) %>% filter(LATITUDE >-10) %>%
  select(PRES_ADJUSTED,LONGITUDE,LATITUDE,Category,WMO,TIME,CYCLE_NUMBER)

# Investigation of Western Pacific events : 
