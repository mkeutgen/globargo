...
downscale_data_fun <- function(df, bin_width = 20, cutoff = 1.96) {
  ...
  downscaled_data <- downscaled_data %>%
    mutate(OUT.S =  ifelse(abs(AOU) > cutoff & abs(ABS_SAL) > cutoff & AOU < 0, 1, 0),
           OUT.T = ifelse(BBP700_ADJUSTED> 0 &
                            abs(BBP700_ADJUSTED) > 1 & abs(AOU) > cutoff &
                            abs(ABS_SAL) > cutoff & AOU < 0, 1, 0))
