...
| `results/` | Reproducible scripts that regenerate the manuscript figures once data products are in place. |
...
- **Manual classifications**: export the latest manual review spreadsheets (e.g., `manual_classification.csv`, `carbon_cat1_class.csv`, `carbon_cat2_class.csv`) into `data/`. These identify which WMO/cycle combinations should be processed for carbon_detection.
...
2. Optional: adjust the hyper-parameters (`cutoff`, `resolution`, `window`) near the top of the script if you want to detect anomalies in other variables (no3 ?).
...
- Large upstream data products (e.g., OCIM-2 sequestration grids, manual classification spreadsheets) are not tracked in Git but should be in zenodo. If not, reach out to corresponding author (me).
